...
def split_nodes_delimiter(old_nodes, delimiter, text_type):
    ...
    for node in old_nodes:
        if node.text_type != TextType.TEXT:
            ...
            pre, inner, post = node.text.split(delimiter, 2)
            if delimiter not in post:
                new_nodes.extend([ TextNode(pre, TextType.TEXT), TextNode(inner, text_type), TextNode(post, TextType.TEXT)])
            else:
                new_nodes.extend([ TextNode(pre, TextType.TEXT), TextNode(inner, text_type) ])
                new_nodes.extend(split_nodes_delimiter([TextNode(post, TextType.TEXT)],delimiter,text_type))
    return new_nodes
...
def markdown_to_blocks(markdown):
    ...
    for block in blocks:
        if block.strip("\n") != "":
            cleanblocks.append(block.strip("\n"))
    return cleanblocks
